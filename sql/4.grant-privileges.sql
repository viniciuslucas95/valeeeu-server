GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.account TO test;

GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.profile TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.profile_contact TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.profile_rating TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.profile_picture TO test;

GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.service_profile TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.service_profile_tag TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.service_profile_item TO test;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public.service_profile_picture TO test;